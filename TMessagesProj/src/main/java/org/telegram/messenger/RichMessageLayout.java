...
public class RichMessageLayout {
    ...
    private void drawBackground(Canvas canvas, boolean hasClip,
                                float clipTop, float clipBottom, float progress) {
        if (!quotes.isEmpty()) {
            for (QuoteBackground q : quotes) {
                if (q.startBlockIndex < 0 || q.startBlockIndex >= blocks.size()
                        || q.endBlockIndex < 0 || q.endBlockIndex >= blocks.size()) {
                    continue;
                }
                final RichBlock first = blocks.get(q.startBlockIndex);
                final RichBlock last = blocks.get(q.endBlockIndex);
                int quoteTop = Math.round(AndroidUtilities.lerp(
                        first.prevY, first.currY, progress));
                int quoteBottom = Math.round(AndroidUtilities.lerp(
                        last.prevY, last.currY, progress)) + last.getHeight();
                if (last.padding.bottom > dp(4)) {
                    quoteBottom -= last.padding.bottom - dp(4);
                }
                if (hasClip && (quoteBottom <= clipTop || quoteTop >= clipBottom)) {
                    continue;
                }
        ...
        for (int i = 0; i < blocks.size(); ++i) {
            ...
            final int top = Math.round(AndroidUtilities.lerp(
                    block.prevY, block.currY, progress));
            final int bottom = top + block.getHeight();
            if (hasClip && (bottom <= clipTop || top >= clipBottom)) {
                continue;
            }
            drawPullquoteBackground(canvas, (RichTextBlock) block, top, bottom);
        }
    }

    private void drawPullquoteBackground(Canvas canvas, RichTextBlock block,
                                         int top, int bottom) {
    ...
    private void drawInternal(Canvas canvas, ChatMessageCell.TransitionParams tp) {
        ...
        final int visiblePartHeight = cell != null ? cell.getVisiblePartHeightForDraw() : 0;
        final boolean hasClip = cell != null && visiblePartHeight > 0;
        if (hasClip) {
            clipTop = cell.getVisiblePartPositionForDraw() - cell.textY;
            clipBottom = clipTop + visiblePartHeight;
        }
    ...
    private void drawInternal(Canvas canvas, ChatMessageCell.TransitionParams tp, boolean hasClip, float clipTop, float clipBottom) {
        final boolean wasDetailsAnimating = detailsAnimating;
        ...
        drawBackground(canvas, hasClip, clipTop, clipBottom, prog);
        ...
        if (wasDetailsAnimating && prog >= 1f) {
    ...
    public boolean drawOverlay(Canvas canvas, ColorFilter colorFilter) {
        if (!isOverlayActive()) return false;
        final int visiblePartHeight = cell != null
                ? cell.getVisiblePartHeightForDraw() : 0;
        final boolean hasClip = cell != null && visiblePartHeight > 0;
        final float clipTop = hasClip
                ? cell.getVisiblePartPositionForDraw() - cell.textY : 0f;
        final float clipBottom = clipTop + visiblePartHeight;
        boolean drew = false;
        for (int i = 0; i < blocks.size(); ++i) {
            final RichBlock block = blocks.get(i);
            if (!block.currVisible) continue;
            if (hasClip && (block.currY + block.getHeight() <= clipTop
                    || block.currY >= clipBottom)) {
                continue;
            }
            canvas.save();
